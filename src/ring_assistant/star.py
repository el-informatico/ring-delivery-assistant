...
def render_star_markdown(run: StarRun) -> str:
    ...
    out.append("")
    out.append("| stage | mode offline | how to go live |")
    out.append("|---|---|---|")
    out.append(
        "| classification | full multimodal call path, deterministic mock model | "
        "set `RING_LLM_ENDPOINT`, `RING_LLM_MODEL`, `RING_LLM_API_KEY` in `.env` |"
    )
    out.append(
        "| delivery | Telegram sink with recording transport (no POST) | "
    ...
    out.append(
        ...
        "network legs (LLM RTT, Telegram RTT) are not included. The artifact is "
